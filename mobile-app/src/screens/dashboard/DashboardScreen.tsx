...
export default function DashboardScreen() {
  ...
  // Prefer structured type/target fields; fall back to the human text so an
  // email notification routes even when it carries no explicit type field.
  function openNotification(n: any) {
    const structured = [n?.type, n?.targetType, n?.targetPath, n?.category, n?.channel]
      .filter(Boolean)
      .join(" ");
    const text = [n?.title, n?.message, n?.body].filter(Boolean).join(" ");
