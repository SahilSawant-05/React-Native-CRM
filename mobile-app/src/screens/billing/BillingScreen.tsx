...
import { AlertTriangle, CheckCircle2, CreditCard, FileText, Gift, HardDrive, History, IndianRupee, Loader2, RefreshCw, ShieldCheck, WalletCards } from "lucide-react";
...
export default function Billing() {
  ...
  const buyStorage = async (storagePackage) => {
    setError("");
    setSuccess("");
    setPurchasingKey(storagePackage.packageKey);
    try {
      await loadRazorpayScript();
      ...
      const checkout = new window.Razorpay({
        ...
        currency: order.currency || "INR",
        name: "Vistaar Flow",
        description: `${formatStorage(order.storageBytes)} storage add-on`,
        order_id: order.razorpayOrderId,
        theme: { color: "#0f766e" },
        handler: async (response) => {
          try {
            ...
            setSuccess(`${formatStorage(order.storageBytes)} extra storage activated.`);
            await load({ quiet: true });
          } catch (verifyError) {
        ...
        <div className="grid gap-3 md:grid-cols-3">
          {storagePackages.map((storagePackage) => (
            <article key={storagePackage.packageKey} className="flex min-h-[210px] flex-col rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
              ...
              <div className="mt-2 text-2xl font-extrabold text-slate-950">{formatCurrency(storagePackage.amountPaise)}</div>
              <div className="mt-1 text-xs font-bold text-teal-700">
                {formatStorage(storagePackage.storageBytes)} for {storagePackage.periodMonths || 12} months
              </div>
              ...
              >
                {purchasingKey === storagePackage.packageKey ? <Loader2 size={16} className="animate-spin" /> : <CreditCard size={16} />}
                Buy Storage
              </button>
            </article>
          ))}
