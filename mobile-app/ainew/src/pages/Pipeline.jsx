...
import { LEAD_SOURCE_VALUES, leadSourceLabel } from "../config/leadSources";
import {
...
const normalizeContactOption = (contact) => ({
  id: contact?.id || contact?._id || "",
  name: contact?.name || contact?.fullName || contact?.email || contact?.phone || "Unknown Contact",
  phone: contact?.phone || contact?.mobile || "",
  email: contact?.email || "",
});

const contactOptionLabel = (contact) => {
  const name = contact?.name || contact?.phone || contact?.email || "Unknown Contact";
...
function Toast({ toast, onClose }) {
  ...
  return (
    <div className={`fixed bottom-4 left-3 right-3 z-50 rounded-lg border px-4 py-3 text-sm shadow-xl sm:bottom-6 sm:left-auto sm:right-6 ${classes}`}>
...
function StageColumn({ stage, cards, totalCount, visibleLimit, onShowMore, onAdd, onCardClick, onEdit, onEmail, onDelete, onDragStart, onDrop }) {
  ...
  return (
    <section
      ...
      className={`flex w-[82vw] max-w-sm flex-shrink-0 snap-start flex-col rounded-xl border p-3 shadow-sm sm:w-80 ${
...
function OpportunityModal({ open, stages, contacts, domainItems, initial, defaultStage, pipeline, saving, onClose, onSave }) {
  ...
  }, [open, initial, defaultStage, stages, pipelineIndustryKey]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        const items = normalizeList(payload).map(normalizeContactOption).filter((contact) => contact.id);
        if (!cancelled) setContactResults(items);
      } catch (error) {
        if (!cancelled) {
          setContactResults([]);
          setContactSearchError(error?.response?.data?.message || error?.message || "Could not search contacts.");
        }
      } finally {
  ...
  const contactOptions = useMemo(() => {
    ...
    const add = (contact) => {
      const next = normalizeContactOption(contact);
      if (next.id) merged.set(String(next.id), next);
    };

    contacts.forEach(add);
    contactResults.forEach(add);
    if (initial?.contactId) {
      add({
        id: initial.contactId,
        name: initial.contactName || "Linked contact",
        phone: initial.contactPhone || "",
      });
    }

    return Array.from(merged.values());
  }, [contacts, contactResults, initial]);
        ...
        <div className="grid min-h-0 flex-1 gap-4 overflow-y-auto px-5 py-4 sm:grid-cols-2">
          <div className="sm:col-span-2">
            <div className="mb-1 flex items-center justify-between gap-2">
              <label className="block text-xs font-semibold uppercase tracking-wide text-gray-500">Contact</label>
              <span className="text-[11px] font-medium text-gray-400">Search by name, phone, or email</span>
            </div>
            <input
              value={contactQuery}
              onChange={(event) => setContactQuery(event.target.value)}
              placeholder="Type at least 2 characters to search contacts"
              className="mb-2 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-teal-500"
            />
            <select
            ...
            <div className="mt-1 min-h-4 text-[11px]">
              {contactSearching && <span className="text-teal-700">Searching contacts...</span>}
              {!contactSearching && contactSearchError && <span className="text-red-600">{contactSearchError}</span>}
              {!contactSearching && !contactSearchError && contactQuery.trim().length >= 2 && (
                <span className="text-gray-400">{contactOptions.length} matching/loaded contacts available</span>
              )}
            </div>
          </div>
          <div className="sm:col-span-2">
          ...
          <div>
            ...
            <div className="min-h-10 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-sm font-semibold text-gray-700">
              {pipeline?.name || "Selected Pipeline"} <span className="font-medium text-gray-400">• Catalog type: {pipelineIndustryKey}</span>
            </div>
          </div>
          <div>
...
export default function ContactPipeline() {
  ...
  const sourceOptions = useMemo(
    () => Array.from(new Set([...LEAD_SOURCE_VALUES, ...allCards.map(sourceFilterValue).filter(Boolean)])).sort(),
    [allCards]
  );
  ...
  const activeFilterCount = useMemo(
    () => Object.entries(filters).filter(([, value]) => value !== "" && value !== false && value !== null && value !== undefined).length
      + (datePreset !== "ALL" ? 1 : 0),
    [datePreset, filters]
  );
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-gray-900">
      <header className="sticky top-0 z-30 border-b border-gray-200 bg-white px-3 py-3 shadow-sm sm:px-4 lg:px-6 lg:py-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h1 className="text-xl font-extrabold tracking-tight text-gray-950 sm:text-2xl">Opportunity Pipeline</h1>
          ...
          <div className="grid gap-2 sm:grid-cols-2 lg:flex lg:flex-wrap lg:items-center">
            <select
              ...
              className="min-h-10 w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm font-semibold text-gray-700 outline-none focus:border-teal-500 lg:w-auto"
            >
            ...
            <input
              ...
              className="min-h-10 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-teal-500 lg:w-64"
            />
            <button
              type="button"
        ...
        <div className="mt-3 flex gap-2 overflow-x-auto pb-1 text-xs text-gray-500 sm:mt-4 sm:flex-wrap sm:overflow-visible sm:pb-0">
          ...
          {selectedPipeline && <span>Catalog type: {selectedPipeline.industryKey || "GENERIC"}</span>}
          <span>Close rate {closeRate}%</span>
          {loading && <span className="text-teal-700">Loading...</span>}
        </div>

        {filtersOpen && (
        <div className="mt-3 grid gap-2 rounded-xl border border-gray-200 bg-gray-50 p-3 sm:mt-4 md:grid-cols-2 xl:grid-cols-[180px_180px_190px_auto_auto]">
          <select
        ...
        {filtersOpen && (
        <div className="mt-3 max-w-xl overflow-x-auto rounded-xl border border-gray-200 bg-gray-50 p-3 sm:overflow-visible">
          <DateRangeFilter
            value={dateRange}
            preset={datePreset}
      ...
      <main className="flex snap-x snap-mandatory gap-3 overflow-x-auto p-3 sm:gap-4 sm:p-4 lg:gap-5 lg:p-6">
        {stages.map((stage) => (
          <StageColumn
            key={stage.key}
            stage={stage}
