...
To: ${email.toEmail || ""}
Linked contact: ${contact?.name || ""}
Linked opportunity: ${opportunity?.title || ""}
Email body:
${textPreview(email.body) || email.body || ""}`}
                    replyPrompt={`Write a professional reply to this email. Keep it concise, helpful, and include one clear next step.
Subject: ${email.subject || ""}
From: ${email.fromEmail || ""}
Linked contact: ${contact?.name || ""}
Linked opportunity: ${opportunity?.title || ""}
Email body:
${textPreview(email.body) || email.body || ""}`}
                    onApply={(text) => setReplyBody((current) => [current, text].filter(Boolean).join(current ? "\n\n" : ""))}
