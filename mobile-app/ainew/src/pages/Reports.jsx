  ...
  return (
    <div className="space-y-4">
      {items.map((item) => {
        ...
        return (
          <div key={item.label}>
            <div className="mb-1 flex min-w-0 flex-col gap-1 text-sm sm:flex-row sm:items-center sm:justify-between sm:gap-3">
              <span className="min-w-0 break-words font-semibold text-gray-800">{formatLabel(item.label)}</span>
              <span className="shrink-0 text-gray-500 sm:text-right">
                {count}
  ...
  return (
    <section className="min-w-0 rounded-lg border border-gray-200 bg-white p-4 shadow-sm sm:p-5">
      <div className="mb-5 flex min-w-0 flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
        ...
        <span className="w-fit shrink-0 rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-700">
          {totalCount} total
        </span>
      </div>
        ...
        <div className="space-y-4">
          {items.map((item) => {
            ...
            return (
              <div key={item.source}>
                <div className="mb-1 flex min-w-0 flex-col gap-1 text-sm sm:flex-row sm:items-center sm:justify-between sm:gap-3">
                  <span className="min-w-0 break-words font-semibold text-gray-800">{formatLabel(item.source)}</span>
                  <span className="shrink-0 text-gray-500 sm:text-right">
  ...
  return (
    <section className="min-w-0 rounded-lg border border-gray-200 bg-white p-4 shadow-sm xl:col-span-2 sm:p-5">
      <div className="mb-5 flex min-w-0 flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
        ...
        <span className="w-fit shrink-0 rounded-full bg-blue-50 px-3 py-1 text-xs font-bold text-blue-700">{items.length} agents</span>
      </div>
      {items.length === 0 ? (
        <div className="rounded-lg border border-dashed border-gray-200 bg-gray-50 px-4 py-8 text-center text-sm text-gray-500">
...
export default function Reports() {
  ...
  if (loading) {
    return <div className="min-h-screen bg-slate-50 p-3 text-sm text-gray-500 sm:p-6">Loading reports...</div>;
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-3 text-gray-900 sm:p-6">
      <div className="mx-auto max-w-7xl">
        <header className="mb-6 rounded-lg border border-gray-200 bg-white p-4 shadow-sm sm:p-5">
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
            <button
              ...
              className="rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm font-extrabold text-gray-700 hover:bg-gray-50"
            >
              {reportFiltersOpen ? "Hide filters" : "Report filters"} {(selectedOwnerUserId || datePreset !== "30D") ? "(active)" : ""}
            </button>
