...
function sectionItems(section, isAdmin) {
  return section.items.filter((item) => (item.adminOnly ? isAdmin : true));
}

function SidebarContent({ collapsed, hideBrand, isAdmin, role, user, unreadNotifications, onNavigate }) {
  return (
    <>
      {!hideBrand && (
        <div className={`mb-5 flex items-start justify-between gap-3 ${collapsed ? "justify-center" : ""}`}>
          {!collapsed && (
            <div>
              <h3 className="text-lg font-extrabold tracking-tight">WhatsApp CRM</h3>
      ...
      <nav className={collapsed ? "space-y-4 pb-6" : "space-y-5 pb-6"}>
        {navSections.map((section) => {
          const items = sectionItems(section, isAdmin);
...
export default function DashboardLayout() {
  ...
  return (
    <div className="flex min-h-screen bg-slate-50">
      ...
      <main className={`min-w-0 flex-1 transition-all duration-200 ${sidebarCollapsed ? "lg:pl-20" : "lg:pl-64"}`}>
        <div className="sticky top-0 z-20 flex h-14 items-center gap-3 border-b border-gray-200 bg-white/95 px-4 backdrop-blur lg:hidden">
          ...
          <div className="min-w-0">
            <div className="truncate text-sm font-extrabold text-gray-950">WhatsApp CRM</div>
