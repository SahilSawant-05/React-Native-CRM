...
export default function Phase2Settings() {
  ...
  const removeEditorStage = (index) => {
    setStageEditorStages((current) =>
      current
        .map((stage, stageIndex) => (stageIndex === index ? { ...stage, active: false } : stage))
        .map((stage, stageIndex) => ({ ...stage, displayOrder: stageIndex + 1 }))
    );
  };
  ...
  const saveEditorStages = async () => {
    ...
    try {
      ...
      setSuccess("Pipeline stages updated");
    } catch (err) {
              ...
              <div className={`grid gap-6 ${showPipelineBuilder ? "xl:grid-cols-[minmax(0,420px)_1fr]" : ""}`}>
                ...
                <section className="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
                  <div className="mb-5 flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-base font-bold text-gray-950">Pipeline Builder</h3>
                      <p className="text-sm text-gray-500">Name it, choose a starter, then publish.</p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      <FieldLabel>Pipeline start type</FieldLabel>
                    ...
                    <div className="grid gap-2 pt-2">
                      <PrimaryButton
                        ...
                        className="w-full"
                      >
                        {saving === "pipeline-template" || saving === "pipeline-blank"
                          ? "Creating..."
                          : pipelineDraftMode === "template"
                            ? "Create Pipeline From Template"
                            : "Create Custom Pipeline"}
                      </PrimaryButton>
                      <SecondaryButton onClick={() => setShowPipelineBuilder(false)} className="w-full">
                        Cancel
                      </SecondaryButton>
                    </div>
                    ...
                    <p className="rounded-lg bg-teal-50 px-3 py-2 text-xs leading-relaxed text-teal-800">
                      {pipelineDraftMode === "template"
                        ? "Templates are only starters. Review and edit stages before creating the pipeline."
                        : "Blank mode starts from your own stage list. Add stages below before creating the pipeline."}
                    </p>
                  </div>
                </section>
                )}
                ...
                <section className="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
                  ...
                  <div className="mb-5 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    <div>
                      <h3 className="text-base font-bold text-gray-950">
                        {pipelineDraftMode === "template" ? "Template Stages Before Create" : "Blank Pipeline Stages Before Create"}
                      </h3>
                      <p className="text-sm text-gray-500">
                        {pipelineDraftMode === "template"
                          ? "These stages come from the selected industry template. Edit them before creating."
                          : "Define your custom stages before publishing this pipeline."}
                      </p>
                    </div>
                  ...
                  <div className={showPipelineBuilder ? "mb-6 border-t border-gray-200 pt-5" : "mb-6"}>
                    <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                      ...
                      <PrimaryButton onClick={() => setShowPipelineBuilder(true)} icon={Plus}>
                        Create Pipeline
                      </PrimaryButton>
                    </div>
                          ...
                          <div className="mb-3 flex items-start justify-between gap-3">
                            <div>
                              ...
                              <div className="mt-1 text-xs text-gray-500">
                                {normalizeIndustryLabel(pipeline.industryKey)} • {pipeline.stageCount || 0} stages • {pipeline.opportunityCount || 0} opportunities
                              </div>
                            </div>
                    ...
                    <div className="mb-6 rounded-lg border border-teal-200 bg-teal-50/40 p-4">
                      <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                        <div>
                          ...
                          <p className="text-sm text-gray-500">Rename, reorder, add, or deactivate stages for this pipeline.</p>
                        </div>
                        <div className="flex flex-wrap gap-2">
                          ...
                          {stageEditorStages.map((stage, index) => (
                            ...
                            >
                              ...
                              <div>
                                ...
                                <SelectInput
                                  value={stage.active === false ? "INACTIVE" : "ACTIVE"}
                                  onChange={(event) => setEditorStageValue(index, "active", event.target.value === "ACTIVE")}
                                >
