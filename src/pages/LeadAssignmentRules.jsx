...
const LEAD_SOURCES = ["WHATSAPP", "WEBSITE_FORM", "WEBSITE", "FACEBOOK", "INSTAGRAM", "GOOGLE_ADS", "REFERRAL", "WALK_IN", "PORTAL", "CAMPAIGN", "OTHER"];
                ...
                {assignableUsers.map((user) => {
                  ...
                  return (
                    <label key={user.id} className="flex items-center gap-2 text-sm text-teal-950">
                      ...
                      />
                      <span>{labelForUser(user)} · {user.role}</span>
                    </label>
                  );
                })}
              </div>
            ...
            <Field label="Criteria Value">
              {form.criteriaType === "LEAD_SOURCE" ? (
                <select value={form.criteriaValue} onChange={(event) => setValue("criteriaValue", event.target.value)} className={inputClass}>
                  ...
                  {LEAD_SOURCES.map((source) => <option key={source} value={source}>{source.replaceAll("_", " ")}</option>)}
                </select>
