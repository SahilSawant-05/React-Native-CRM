...
const toColumns = (tasks) => {
  ...
  return STATUS_COLUMNS.map((column) => ({ ...column, cards: map[column.id] || [] }));
};
...
const inputCls = "w-full px-3.5 py-2.5 bg-white/[0.04] border border-[#30363d] rounded-xl text-sm text-[#e6edf3] outline-none focus:border-emerald-500/60 transition-colors placeholder:text-[#484f58]";
    ...
    <Modal open={open} onClose={onClose} title={isEdit ? "Update Task" : "Add New Task"}>
      ...
      <div className="grid grid-cols-2 gap-4">
            ...
            {["low","medium","high"].map(p => {
              ...
              return (
                <button key={p} onClick={() => set("priority", p)}
                  className={`flex-1 py-2 rounded-lg text-xs font-semibold    border transition-all ${active ? meta.badge : "border-[#30363d] text-[#7d8590] hover:border-[#484f58]"}`}>
                  {meta.label}
                </button>
              );
            })}
        ...
        <Field label="Due Date & Time">
          <input type="datetime-local" value={form.date} onChange={e => set("date", e.target.value)} className={`${inputCls} [color-scheme:dark]`} />
        </Field>
      </div>
      ...
      <Field label="Tags">
        <div className="flex flex-wrap gap-1.5 mb-2">
          {form.tags.map(t => (
            <span key={t} className="flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-emerald-500/15 text-emerald-400 text-xs    border border-emerald-500/30">
              {t}
              <button onClick={() => toggleTag(t)} className="hover:opacity-70 ml-0.5">×</button>
            </span>
          ))}
        </div>
        <div className="flex flex-wrap gap-1.5">
          {TAGS.filter(t => !form.tags.includes(t)).map(t => (
            <button key={t} onClick={() => toggleTag(t)} className="px-2.5 py-0.5 rounded-full border border-[#30363d] text-[#7d8590] text-xs    hover:border-emerald-500/40 hover:text-emerald-400 transition-colors">
              {t}
            </button>
          ))}
        </div>
      ...
      <div className="flex gap-2 justify-end mt-2">
        <button onClick={onClose} className="px-5 py-2.5 rounded-xl border border-[#30363d] text-[#7d8590] hover:text-[#e6edf3] text-sm transition-colors">Cancel</button>
...
export default function TaskKanban() {
    ...
    try {
      if (editCard) {
        ...
        setColumns(prev => prev.map(col => {
          ...
          if (col.id === (data.colId || activeColId)) return { ...col, cards: [...without, merged] };
        ...
        setColumns(prev => prev.map(col => col.id === (data.colId || activeColId) ? { ...col, cards: [...col.cards, newCard] } : col));
  ...
  return (
    <div className="min-h-screen bg-white text-[#e6edf3]">
      ...
      {/* ── Topbar ── */}
      <div className="sticky top-0 z-40 bg-[#0d1117]/90 backdrop-blur-md border-b border-[#30363d] px-6 h-14 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setContactModal(true)}
            className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border text-xs font-semibold    transition-all ${contactId ? "border-emerald-500/50 bg-emerald-500/10 text-emerald-400" : "border-[#30363d] text-black hover:border-white hover:text-gray-200"}`}>
      ...
      {/* ── Filter bar ── */}
      <div className="px-6 h-11 flex items-center gap-2 border-b border-[#30363d] bg-[#0d1117]/80 backdrop-blur-sm">
      ...
      {!contactId && !activeFilter && !apiLoading ? (
        ...
        <div className="bg-white flex gap-5 p-6 overflow-x-auto items-start min-h-[calc(100vh-88px)] pb-12">
