...
const LEAD_SOURCE_OPTIONS = ["WHATSAPP", "EMAIL", "WEB", "IMPORT", "MANUAL", "REFERRAL", "FACEBOOK"];
...
const ContactSidebar = ({
  ...
  return (
    <aside className="w-72 flex-shrink-0 bg-white rounded-2xl shadow-md border border-gray-100 flex flex-col overflow-hidden">
          ...
          <div className="flex gap-3 mt-2">
            {[
              ...
              <button
                ...
                title={label}
                onClick={() =>
                  label === "Chat" && navigate("/dashboard/chat", { state: { contact } })
                }
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3 text-sm">
        ...
        {[
          ...
          { label: "Source", value: contact.leadSource?.replaceAll("_", " ") },
          { label: "Source Detail", value: contact.leadSourceDetail },
          { label: "Industry", value: contact.industryKey?.replaceAll("_", " ") },
    ...
    <div className="p-3 sm:p-4 md:p-6">
      <div className="mb-4 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
        <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
          <div className="grid flex-1 gap-3 md:grid-cols-[minmax(220px,1fr)_220px_220px]">
            ...
            <select
              value={filters.industryKey}
              onChange={(event) => updateFilter("industryKey", event.target.value)}
              className="rounded-xl border-2 border-gray-200 bg-white px-3 py-2 text-sm focus:border-teal-400 focus:outline-none"
            >
              <option value="">All industries</option>
              {INDUSTRY_OPTIONS.map((industry) => (
                <option key={industry} value={industry}>{industry.replaceAll("_", " ")}</option>
              ))}
            </select>
        </div>
                  ...
                  contacts.map((contact, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-gray-500 whitespace-nowrap">
                          {contact.leadSource ? contact.leadSource.replaceAll("_", " ") : "—"}
                        </td>
                        <td className="px-4 py-3">
                          <ScoreBadge score={contact.lead_score} />
                        </td>
      ...
      {showSidebar && selectedContact && (
        <div className="sm:hidden fixed inset-0 z-50 flex items-end">
          ...
          />
          <div className="relative w-full max-h-[80vh] overflow-y-auto rounded-t-2xl bg-white shadow-2xl">
            <ContactSidebar
              contact={selectedContact}
              workspace={workspace}
              customFields={customFields}
