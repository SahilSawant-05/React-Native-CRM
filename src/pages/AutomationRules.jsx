...
const leadSourceOptions = ["WHATSAPP", "EMAIL", "WEBSITE", "FACEBOOK", "INSTAGRAM", "GOOGLE_ADS", "CSV_IMPORT", "REFERRAL", "MANUAL"];
            ...
            {rules.map((rule) => (
              <article key={rule.id} className="flex flex-col gap-3 px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
                ...
                <div className="flex gap-2">
                  ...
                  <button type="button" onClick={() => deleteRule(rule)} className="rounded-lg border border-red-200 px-3 py-2 text-sm font-semibold text-red-600 hover:bg-red-50">Delete</button>
                </div>
              </article>
            ))}
          ...
          <>
            <Field label="Lead source filter">
              <select value={form.conditionLeadSource} onChange={(event) => setValue("conditionLeadSource", event.target.value)} className={inputClass}>
                ...
                {leadSourceOptions.map((source) => <option key={source} value={source}>{labelFor(source)}</option>)}
              </select>
            </Field>
...
function detailSummary(form, stageOptions, pipelines = []) {
  ...
  if (form.actionType === "CREATE_OPPORTUNITY") {
    return `${form.targetPipelineId ? pipelineName(pipelines, form.targetPipelineId) : "Default pipeline"} / ${form.targetStage ? stageLabel(stageOptions, form.targetStage) : "New"}`;
  }
  if (form.actionType === "MOVE_OPPORTUNITY_STAGE") {
